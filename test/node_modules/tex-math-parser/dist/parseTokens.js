...
function createMathJSNode(token, children = []) {
    let fn = Token_1.typeToOperation[token.type];
    switch (token.type) {
        case 6 /* Times */:
            return new customMath_1.default.FunctionNode('cross', children);
        case 4 /* Minus */:
        ...
        case 18 /* Frac */:
        case 7 /* Slash */:
            return new customMath_1.default.OperatorNode(token.lexeme, fn, children);
        case 8 /* Caret */:
            if (children.length < 2) {
                throw new ParseError_1.default('Expected two children for ^ operator', token);
            }
            // manually check for ^T as the transpose operation
            if (children[1].isSymbolNode && children[1].name === 'T') {
                return new customMath_1.default.FunctionNode('transpose', [children[0]]);
            }
            return new customMath_1.default.OperatorNode(token.lexeme, fn, children);
        ...
        case 46 /* Norm */:
        case 47 /* Inv */:
            return new customMath_1.default.FunctionNode(fn, children);
        case 2 /* Equals */:
            return new customMath_1.default.AssignmentNode(children[0], children[1]);
        case 1 /* Variable */:
            return new customMath_1.default.SymbolNode(token.lexeme);
        case 0 /* Number */: {
            // convert string lexeme to number if posssible
            const constant = Number.isNaN(Number(token.lexeme)) ? token.lexeme : +token.lexeme;
            return new customMath_1.default.ConstantNode(constant);
        }
        case 30 /* Pi */:
            return new customMath_1.default.SymbolNode('pi');
        case 31 /* E */:
            return new customMath_1.default.SymbolNode('e');
        case 34 /* Matrix */:
            return new customMath_1.default.ArrayNode(children);
        case 38 /* T */:
            return new customMath_1.default.SymbolNode('T');
        default:
            throw new ParseError_1.default('unknown token type', token);
    }
}
...
class Parser {
    ...
    nextExpression() {
        ...
        if (this.match(2 /* Equals */)) {
            if (!leftTerm.isSymbolNode) {
                throw new ParseError_1.default('expected variable (SymbolNode) on left hand of assignment', this.previousToken());
            }
    ...
    nextTerm() {
        ...
        for (;;) {
            ...
            if (isNumberNode(leftFactor) && lookaheadType === 0 /* Number */) {
                throw new ParseError_1.default('multiplication is not implicit between two different'
                ...
                operator = new Token_1.default('*', 5 /* Star */, starPos);
                implicitMult = true;
            }
    ...
    tryConsume(errMsg, ...tokenTypes) {
        ...
        if (lookaheadType === undefined) {
            throw new ParseError_1.default(errMsg, this.currentToken());
        }
    ...
    nextPrimary() {
        ...
        if (lookaheadType === undefined) {
            throw new ParseError_1.default('expected primary', this.currentToken());
        }
        let primary;
        switch (lookaheadType) {
            ...
            case 32 /* Begin */:
                ...
                break;
            default:
                throw new ParseError_1.default('unknown token encountered during parsing', this.nextToken());
        }
        return primary;
    }
    /**
    ...
    nextGrouping() {
        ...
        if (this.match(35 /* Left */)) {
            leftRight = true;
            this.nextToken(); // consume \left
        }
        const leftGrouping = this.tryConsume("expected '(', '|',", 12 /* Lparen */, 14 /* Bar */);
        ...
        if (leftRight) {
            this.tryConsume('expected \\right to match corresponding \\left after expression', 36 /* Right */);
        }
    ...
    nextArgument() {
        ...
        if (this.match(35 /* Left */, 12 /* Lparen */, 10 /* Lbrace */, 14 /* Bar */)) {
    ...
    nextMatrix() {
        ...
        for (;;) {
            ...
            if (this.match(15 /* Amp */)) {
            ...
            else if (this.match(16 /* Dblbackslash */, 33 /* End */) !== undefined) {
                ...
                row = [];
                if (delimiter.type === 33 /* End */) {
                    break;
                }
            }
            else if (this.match(37 /* Eof */)) {
                throw new ParseError_1.default('unexpected EOF encountered while parsing matrix', this.currentToken());
            }
            else {
                throw new ParseError_1.default('unexpected delimiter while parsing matrix', this.currentToken());
            }
        }
    ...
    tryConsumeRightGrouping(leftGroupingToken) {
        ...
        const expectedLexemes = Object.keys(Token_1.lexemeToType)
            .filter((key) => Token_1.lexemeToType[key] === rightGroupingType)
            // insert quotes (e.g. { => '{')
            .map((lexeme) => `'${lexeme}'`);
